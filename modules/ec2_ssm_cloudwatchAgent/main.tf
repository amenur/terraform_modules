...
resource "local_file" "ssh_key" {
  count = var.enable_ssh_key_pem_local || (var.enable_ssh_key_pem_local < 1 && var.enable_ssh_key_ssm_param_store < 1) ? 1 : 0
...
data "aws_security_group" "public_sg" {
  count = var.enable_private_sg && var.enable_public_sg < 1 ? 1 : 0
...
#  ami           = (var.enable_self_ami > 0 && var.enable_new_ami < 1) || (var.enable_self_ami < 1 && var.enable_new_ami < 1) || (var.enable_self_ami > 0 && var.enable_new_ami > 0) ? data.aws_ami.self_ami.id : data.aws_ami.new_ami.id
  ami = (var.enable_self_ami != 0 && var.enable_new_ami > 0) ? data.aws_ami.new_ami.id : data.aws_ami.self_ami.id
